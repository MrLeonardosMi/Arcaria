...
export class BlastingRecipe extends Recipe {
  ...
  generateScript(): string {
    ...
    switch (this.config.modType) {
      case 'kubejs':
        script = `event.blasting("${this.output}", "${input}");`;
        break;
      
      case 'crafttweaker':
        script = `blastFurnace.addRecipe(${wrapMineTweakerMeta(this.output)}, ${wrapMineTweakerMeta(input)});`;
        break;
      
      case 'minetweaker':
        throw new Error('Blasting recipes are not supported in MineTweaker');
